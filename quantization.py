...
# 构建自定义求导方法的神经网络时，需继承 torch.autograd.Function 类，此时需要重写静态的 forward 函数和 backward 函数，其中 backward 函数的作用是返回求导结果。在前向传播中使用这个自定义神经网络时，调用类方法 apply()，不要直接调用 forward()。
class W8Linear(torch.autograd.Function):

    @staticmethod
    def forward(ctx, x, weight, bias):
        # 静态方法中，带ctx参数而不是self。调用apply()传入的ctx可以利用save_for_backward来保存tensor，在backward阶段可以获取保存的tensor
        ctx.save_for_backward(x, weight, bias)
        # print(weight.shape, x.dtype)
        ...
        # forward的返回和backward的输入值应该是按顺序对应的，反之亦然
        return output
    ...
    def backward(ctx, grad_output):
        ...
        def backward_w_float_weight(weight, grad_output):
            ...
            # print("------------float_weight.dtype", float_weight.dtype)
        ...
        # gradient accumulation 注意为pytorch的整数类型参数设置float类型的梯度会报错
        if not hasattr(weight, 'float_grad'):
            weight.__setattr__('float_grad', None)

        if weight.float_grad is not None:
            # print("weight.float_grad:", weight.float_grad.dtype)
...
def prepare_model_for_int8_training(model, args, target_module):
    ...
    for name, module in reversed(model._modules.items()):   # model._modules是一个dict，内容和dict化的model.named_children()差不多
        ...
        if isinstance(module, nn.Linear):
            ...
            new_layers = QLinear(module.weight, bias_data, num_bits=args.weight_bits, group_size=args.weight_group_size, stochastic_round=args.stochastic_round)
...
if __name__ == '__main__':
    ...
    int8_linear1 = QLinear(fp16_linear1.weight, None, device='cuda:1', num_bits=8, group_size=GROUP_SIZE)
